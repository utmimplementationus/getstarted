...
During the task force deliberations, the FAA shared a more complex prioritization scheme than the task force ultimately recommended.  As a matter of public policy, the FAA shared this recommendation in the Normalizing UAS BVLOS Operations Notice of Proposed Rulemaking. 

### Initial Implementation

This policy is drafted to support the task force’s recommendation of two initial operational priority levels that correspond to the most common use cases seen by operators using UTM today. Based on emerging operational needs and forthcoming regulations, this policy and associated implementation materials can be updated on an as needed basis.
...
* Training missions
* Overwatch, surveillance or patrols where safety of life is _not_ at stake.
...
SDD v2.0.1 and RTM v2.0 were approved by the Technical Committee to enable high priority operations, and the monthly reporting template was updated to v2. 
...
Operators who intend to fly using high priority are required to conduct Operational Practicality simulations with overlapping stakeholders before submitting the issue ticket below. The Operational Practicality simulation must reasonably represent the proportion, frequency, duration and operational intent geometry of high-priority and standard-priority missions that the operator expects to conduct. The acceptance process for the Operational Practicality simulations is defined in Annex C, Sec. 5(c). The above provisions apply regardless of whether the operator has previously completed onboarding. The intent of this policy is to collect as much infomration and data as possible about the airspace usage and operational impacts of high-priority operations, so that approvals can be granted with sufficient understanding of the impact of the propsoed change, compared with equal-priority operations. 
...
The v2 monthly reporting template supports data collection for prioritization based on metrics recommended by the Prioritization Task Force.
